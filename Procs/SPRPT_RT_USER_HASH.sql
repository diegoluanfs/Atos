...
IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE TYPE = 'P' AND OBJECT_ID = OBJECT_ID('[DBO].[SPRPT_RT_USER_HASH]'))
   EXEC('CREATE PROCEDURE [DBO].[SPRPT_RT_USER_HASH] AS BEGIN SET NOCOUNT ON; END')
GO
...
-------------------------------------------------------------------------------------------------------------------------  
 ...
 EXEC [dbo].[SPRPT_RT_USER_HASH]  
           
-------------------------------------------------------------------------------------------------------------------------  
  
*/   
  
ALTER PROCEDURE [dbo].[SPRPT_RT_USER_HASH]  
(    
 @ID_USER     INT    = NULL  
)  
AS  
...
SELECT [HASH_USER]  AS [HASH]
  FROM [dbo].[USERS] (NOLOCK)  
  WHERE ID = ISNULL (@ID_USER,[ID])
END TRY  
